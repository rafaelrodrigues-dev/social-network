...
class NewPublicationIntegrationTest(DjangoTestCase):
    ...
    def test_create_publication_view_renders_form(self):
        url = reverse('publications:create-publication')
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'publications/pages/create_publication.html')
        self.assertIn('form', response.context)

    def test_create_publication_valid_data(self):
        url = reverse('publications:create-publication')
